...
namespace Fiorello.Areas.admin.Controllers
{
	...
	public class ProductController : Controller
	{
		...
		private async Task<List<Product>> FilterProductsAsync(ProductIndexVM model)
		{
			...
			products = FilterByCategory(products, model.CategoryIds);
		...
		{
			return products
							.Where(p => minStock != null ? p.Stock >= minStock : true &&
										maxStock != null ? p.Stock <= maxStock : true);
		}
		private IQueryable<Product> FilterByCategory(IQueryable<Product> products,List<int> categoryIds)
		{
			return products.Where(p => categoryIds.Count == 0 ? true : categoryIds.Contains(p.ProductCategoryId));
		}
		...
		{
			return products.Where(p => createdAtStart != null ? p.CreatedAt > createdAtStart.Value.Date : true &&
											createdAtEnd != null ? p.CreatedAt < createdAtEnd.Value.Date : true);
		}
		#endregion
